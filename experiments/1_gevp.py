...
solver_rsd = RiemmGeneralizedStiefel(A, B, p, maxiter = 4000, mingradnorm=1e-6,maxtime=max_time)
x_rsd, optlog_rsd = solver_rsd.solve(eta = .001, step_type = 'fixed', x0=x0)

solver_land_riem = GeneralizedLanding(A, B, p, maxiter = 4000, mingradnorm=1e-6,maxtime=max_time)
x_land_riem, optlog_land_riem = solver_land_riem.solve(.001, 1, grad_type='riem', x0=x0, step_type='fixed')

solver_land_precon = GeneralizedLanding(A, B, p, maxiter = 4000, mingradnorm=1e-6,maxtime=max_time)
x_land_precon, optlog_land_precon = solver_land_precon.solve(170, 0.1, grad_type='precon', x0=x0, step_type='fixed')
