...
x_land_R, optlog_land_R = solver_land_R.solve(0.001*coef, 17000/coef, grad_type='R', x0=x0, step_type='fixed', eps_d = 0.5)

solver_plam = GeneralizedLanding(A, B, p, maxiter = maxiter, mingradnorm=1e-6,maxtime=max_time)
x_plam, optlog_plam = solver_plam.solve(.05*coef, 200/coef, grad_type='plam', x0=x0, eps_d = None, step_type='fixed')



solver_land_precon = GeneralizedLanding(A, B, p, maxiter = maxiter, mingradnorm=1e-6,maxtime=max_time)
x_land_precon, optlog_land_precon = solver_land_precon.solve(170*coef, 0.1/coef, grad_type='precon', x0=x0, step_type='fixed', eps_d = 1)


solver_rsd = RiemmGeneralizedStiefel(A, B, p, maxiter = maxiter, mingradnorm=1e-6,maxtime=max_time)
x_rsd, optlog_rsd = solver_rsd.solve(eta = .005*coef, step_type = 'fixed', x0=x0)

solver_land_riem = GeneralizedLanding(A, B, p, maxiter = maxiter, mingradnorm=1e-6,maxtime=1)
x_land_riem, optlog_land_riem = solver_land_riem.solve(.001*coef, 1/coef, grad_type='riem', x0=x0, step_type='fixed', eps_d = 1)


# solver_land_R = GeneralizedLanding(A, B, p, maxiter = maxiter, mingradnorm=1e-6,maxtime=max_time)
# x_land_R, optlog_land_R = solver_land_R.solve(0.001*coef, 10000/coef, grad_type='R', x0=x0, step_type='fixed', eps_d = 1)

# solver_land_precon = GeneralizedLanding(A, B, p, maxiter = maxiter, mingradnorm=1e-6,maxtime=max_time)
# x_land_precon, optlog_land_precon = solver_land_precon.solve(200*coef, 0.1/coef, grad_type='precon', x0=x0, step_type='fixed', eps_d = 1)

# solver_plam = GeneralizedLanding(A, B, p, maxiter = maxiter, mingradnorm=1e-6,maxtime=max_time)
# x_plam, optlog_plam = solver_plam.solve(.5*coef, 200/coef, grad_type='plam', x0=x0, eps_d = None, step_type='fixed')

# solver_land_riem = GeneralizedLanding(A, B, p, maxiter = maxiter, mingradnorm=1e-6,maxtime=max_time)
# x_land_riem, optlog_land_riem = solver_land_riem.solve(.001*coef, 1/coef, grad_type='riem', x0=x0, step_type='fixed', eps_d = 1)

# solver_rsd = RiemmGeneralizedStiefel(A, B, p, maxiter = maxiter, mingradnorm=1e-6,maxtime=max_time)
# x_rsd, optlog_rsd = solver_rsd.solve(eta = .001*coef, step_type = 'fixed', x0=x0)
